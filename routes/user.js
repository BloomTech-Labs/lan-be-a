...
app.get("/", async (request, response) => {
  try {
    // Corey: Added try...catch due to UnhandledPromise err
...
app.put("/displayname", verifyUser, (request, response) => {
  ...
  User.update(userID, { display_name: displayName })
    .then(() => {
      response
        .status(200)
        .json({ message: "Updated user's GitHub username successfully" });
    })
    .catch((err) => {
      console.log(err);
      response
        .status(500)
        .json({ message: "Error updating user's Github Username" });
    });
});
...
app.put("/onboard", verifyUser, (request, response) => {
  ...
  User.onboard(userID)
    .then(() =>
      response
        .status(200)
        .json({ message: "Updated user's onboarded field successfully" })
    )
    .catch((err) => {
      console.log(err);
      response
